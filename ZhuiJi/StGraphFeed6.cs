...
public class StGraphFeed6 : MonoBehaviour
{
    float lastTime = 0f;
    ...
    void MyAddPoint()
    {
        ...
        if (lastTime + 0.01f <= curTime)
        {
            ...
            if (MyTimer6.isStart == true)
            {
                    ...
                    yy = 3.0 * MyTimer6.m_Timer;
                    graph.DataSource.AddPointToCategory2("Player 1", (MyTimer6.m_Timer), yy); // each time we call AddPointToCategory
                                                                                              //distancetext.text = yy.ToString("f2") + "m";




                yy = 7.5 *( MyTimer6.m_Timer-15);
                graph.DataSource.AddPointToCategory2("Player 2", (MyTimer6.m_Timer), yy); // each time we call AddPointToCategory
                //distancetext.text = yy.ToString("f2") + "m";
            }
